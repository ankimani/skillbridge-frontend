...
import CustomHeader from '../../features/shared/CustomHeader';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('JobChat: Fetching messages for jobId:', jobId, 'userId:', profile.userId);
        console.log('JobChat: User profile:', profile);
        const response = await getMessagesForUser(jobId, profile.userId, token);
        console.log('JobChat: Messages response:', response.data);
        console.log('JobChat: Messages array length:', response.data?.length || 0);
