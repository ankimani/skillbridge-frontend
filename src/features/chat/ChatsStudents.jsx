...
const ChatsStudents = () => {
  ...
  const handleChatClick = (jobId, recipientId) => {
    console.log('ChatsStudents: Navigating to chat with jobId:', jobId, 'recipientId:', recipientId);
    console.log('ChatsStudents: Current token exists:', !!token);
    console.log('ChatsStudents: Current user profile:', userProfile);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const token = useAuthStore.getState().token || localStorage.getItem('authToken');
        console.log('ChatsStudents: Token exists:', !!token);
        ...
        if (!checkTokenValid()) {
          console.log('ChatsStudents: Token is invalid, forcing logout');
          useAuthStore.getState().forceLogout();
          navigate('/login');
          return;
        }
        ...
        console.log('ChatsStudents: User profile fetched:', userProfile);
        ...
        const messages = await getChatList(userProfile.userId, token);
        ...
        if (messages && messages.length > 0) {
          ...
          const jobDetailsPromises = sortedMessages.map(async (chat) => {
            const jobDetail = await fetchJobDetails(chat.jobId, token);
            return { [chat.jobId]: jobDetail };
          });
          ...
          // Fetch usernames for all recipients
          await fetchUsernames(sortedMessages, token);
  ...
  const filteredChats = useMemo(() => {
    if (activeTab === 'all') return chatList;
    if (activeTab === 'unread') return chatList.filter(chat => chat.unreadCount > 0);
    return chatList;
  }, [chatList, activeTab]);
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/30 p-16 text-center">
              ...
              <p className="text-gray-500 mb-8 text-lg">
                {activeTab === 'unread' 
                  ...
                  : 'Start connecting with professionals to see your conversations here.'}
              </p>
              <button 
